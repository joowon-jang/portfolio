...
const LINKS = {
  ...
  p2: [
    { icon: '🌐', key: 'live', url: 'https://tododong.com' },
    { icon: '🐙', key: 'repo', url: 'https://github.com/joowon-jang/tododong' }
  ],
  p3: [
    { icon: '🌐', key: 'live', url: 'https://haemadi.vercel.app' },
    { icon: '🐙', key: 'repo', url: 'https://github.com/joowon-jang/haemadi' }
  ]
};

const LINK_LABELS = {
  ko: { live: '배포 사이트', repo: '프론트엔드 저장소', backend: '백엔드 저장소' },
  en: { live: 'Live Site', repo: 'Frontend Repo', backend: 'Backend Repo' },
  ja: { live: '公開サイト', repo: 'フロントエンド', backend: 'バックエンド' }
};
...
const I18N = {
  ko: {
    ...
    dShots: '실제 서비스 화면',
    dOutcome: '검증된 결과',
    dTodo: '※ 상세 내용은 예시로 작성된 부분이 있습니다. 링크(mock 주소)·스크린샷·세부 지표를 실제 자료로 교체해 주세요.',
    projects: [
      { id: 'p1', name: '페이스리뷰', badge: '🏆', desc: '웹캠 표정을 다섯 감정으로 분석해 시청 반응과 영상 추천으로 연결한 서비스', period: '2023.10 ~ 2023.12 · 2026.01 ~ 2026.04 · 2026.07', team: '최초 개발 4인 · 리팩토링 2인', verified: true,
        list: {
          contribution: '2023 프론트엔드·감정 모델 학습 · 2026 프론트엔드 리팩토링',
          implementation: '감정 타임라인·영상 컨트롤 · 검색/즐겨찾기 · 반응형/접근성/SSG'
        },
        productSummary: 'YouTube 시청 중 웹캠 프레임을 서버로 보내고, AI Hub 감정인식 데이터셋으로 직접 학습한 모델이 표정을 행복·놀람·분노·슬픔·무표정으로 분류합니다. 구간별·영상별로 누적한 감정 데이터는 현재 구간의 시청자 반응, 감정 타임라인, 개인 통계, 영상 추천에 사용됩니다.',
        ...
        roles: [
          { title: '2023 · 최초 개발 (4인)', desc: 'Frontend로 참여했고, 서비스에 사용한 행복·놀람·분노·슬픔·무표정의 다섯 감정 분류 모델을 직접 학습했습니다.' },
          { title: '2026.01 ~ 04 · 1차 리팩토링 (2인)', desc: '프론트엔드를 맡아 개선 방향과 신규 기능을 기획하고, CRA·Yarn Classic을 Vite 7·Yarn Berry와 React 19·TypeScript 5.9 환경으로 전환했습니다.' },
          { title: '2026.07 · 2차 리팩토링 (2인)', desc: '감정 타임라인과 커스텀 영상 컨트롤, 검색·즐겨찾기를 구현하고 3단계 반응형 UI, 접근성, TanStack Query, SSG·영상별 OG, ErrorBoundary를 정비했습니다. AGENTS.md·DESIGN.md·API 스킬로 AI 에이전트의 작업 기준을 만들고 커밋 단위로 결과를 검수했습니다.' }
        ],
        troubles: [
          { problem: '저장된 영상 길이와 실제 플레이어 길이가 달라 100개 감정 구간과 재생 위치가 어긋나는 문제', solution: '플레이어가 반환하는 실제 duration을 기준으로 각 구간의 중심점을 초 단위 좌표로 변환해 그래프 시간축과 시킹 위치를 맞췄습니다.' },
          { problem: '타임라인을 드래그할 때마다 시크 명령이 발생해 영상 미리보기가 버벅이는 문제', solution: '시크 호출을 200ms 간격으로 제한하면서 마지막 위치는 반드시 반영하고, 드래그 전 재생·정지 상태도 유지했습니다.' },
          { problem: '여러 API가 동시에 401을 받으면 토큰 재발급 요청이 중복되고 인증 상태가 꼬이는 문제', solution: '재발급 요청은 하나만 실행하고 나머지 요청은 대기시킨 뒤, 갱신된 토큰으로 함께 재시도하도록 구성했습니다.' }
        ],
        ...
        stackGroups: [
          { name: '프론트엔드 · 직접 담당', items: ['React 19', 'TypeScript 5.9', 'Vite 7', 'TanStack Query v5', 'Zustand', 'Socket.IO Client', 'Nivo', 'SCSS', 'Vitest', 'Vercel'] },
          { name: '백엔드 · AI 파이프라인', items: ['Flask 3', 'Flask-SocketIO', 'TensorFlow', 'OpenCV'] },
          { name: '데이터 · 인프라', items: ['MariaDB', 'MongoDB', 'Redis', 'Docker'] }
        ],
        outcome: '2023년 세종대학교 제16회 창의설계경진대회에서 대상을 수상했습니다. 2026년에는 프론트엔드 리팩토링 버전을 facereview.net에 배포했습니다.' },
      { id: 'p2', name: '토도동', badge: '🏀', desc: 'NBA 경기의 실시간 문자 중계와 경기별 채팅을 함께 제공하는 플랫폼', period: '2025.02 ~ 2025.05', team: '팀 프로젝트 (4인)',
        ...
        stack: ['Next.js', 'TypeScript', 'Socket.io', 'Clean Architecture'],
      ...
      { id: 'p3', name: '해마디', badge: '📔', desc: '하루의 감정을 기록하고 그 흐름을 시각화해 돌아보는 일기 서비스', period: '2024.08 ~ 2024.11', team: '팀 프로젝트 (3인)',
        list: { contribution: '일기 작성·감정 시각화 UI · Zustand 상태 구조 설계', implementation: '반응형 분기 일원화 · 공통 Framer Motion variants' },
        bullets: ['하루의 감정을 기록하고 돌아볼 수 있는 감정 관리 일기 서비스입니다.', '일기를 쓰면 감정이 바다의 "마디"로 시각화되어, 시간이 지날수록 나만의 감정 지도가 만들어집니다.', 'Zustand와 CSS media query를 활용해 유지보수 가능한 반응형 UI를 구현했습니다.', 'Framer Motion으로 사용자 상호작용에 따른 동적 애니메이션을 구현해 기록하는 경험 자체가 즐겁도록 만들었습니다.'],
        flow: ['감정 태그 선택', '하루 기록 작성', '색·형태로 감정 시각화', '월간 감정 흐름 회고'],
        features: [
          { name: '감정 일기 작성', desc: '감정 태그와 함께 하루를 기록 — 작성 과정에 맞춘 단계별 애니메이션' },
          { name: '감정 시각화', desc: '기록된 감정을 색과 형태로 시각화해 한 달의 감정 흐름을 한눈에' },
          { name: '반응형 UI', desc: '모바일·태블릿·데스크톱 모두 대응하는 반응형 레이아웃' },
          { name: '인터랙션 애니메이션', desc: 'Framer Motion 기반 페이지 전환·제스처 반응 애니메이션' }
        ],
        roles: ['프론트엔드 구현 — 일기 작성 플로우, 감정 시각화 UI', 'Zustand 상태 설계 — 화면 크기별 분기 로직을 스토어로 통합해 유지보수성 확보', 'Framer Motion 애니메이션 시스템 구축 — 공통 트랜지션 변형(variants) 정의로 일관된 모션 언어 적용'],
        troubles: [
          { problem: '화면 크기별 분기가 컴포넌트마다 흩어져 반응형 코드가 유지보수하기 어려워진 문제', solution: 'media query 기준을 토큰화하고 Zustand 스토어에서 뷰포트 상태를 일원 관리해, 분기 로직 중복을 제거했습니다.' },
          { problem: '페이지 전환 애니메이션이 컴포넌트 언마운트와 충돌해 모션이 뚝 끊기는 문제', solution: 'AnimatePresence로 exit 애니메이션을 보장하고 공통 variants를 정의해 전환이 자연스럽게 이어지도록 했습니다.' }
        ],
        stack: ['React', 'Zustand', 'Framer Motion'],
        outcome: '일기 작성 플로우와 감정 시각화 UI를 구현하고, 반응형 상태 구조와 공통 모션 시스템을 정리했습니다.' }
    ]
  },
  en: {
    ...
    enter: 'DETAILS',
    lockedQuest: 'New project — coming soon',
    ...
    blogTitle: '■ TECH BLOG',
    blogPosts: [],
    contactTitle: '■ CHANNELS — EMAIL · REPOS · BLOG',
    ...
    dTodo: '※ Some details are illustrative examples. Replace the mock links, screenshots, and detailed metrics with real material.',
    projects: [
      { id: 'p1', name: 'Face Review', badge: '🏆', desc: 'A service that turns five emotions inferred from webcam expressions into viewing reactions and video recommendations', period: '2023.10 ~ 2023.12 · 2026.01 ~ 2026.04 · 2026.07', team: 'Original team of 4 · Refactoring team of 2', verified: true,
        list: {
          contribution: '2023 frontend and emotion-model training · 2026 frontend refactoring',
          implementation: 'Emotion timeline and video controls · search/bookmarks · responsive UI, accessibility, and SSG'
        },
        productSummary: "While a viewer watches YouTube, webcam frames are sent to the server and a model trained directly on AI Hub's emotion-recognition dataset classifies happy, surprise, angry, sad, and neutral. Emotion data accumulated by segment and video powers the current audience reaction, timeline, personal statistics, and recommendations.",
        ...
        roles: [
          { title: '2023 · Original build (team of 4)', desc: 'Worked on the frontend and directly trained the five-emotion classifier—happy, surprise, angry, sad, and neutral—used by the service.' },
          { title: '2026.01–04 · First refactoring (team of 2)', desc: 'Owned the frontend, planned improvements and new features, and migrated CRA and Yarn Classic to Vite 7, Yarn Berry, React 19, and TypeScript 5.9.' },
          { title: '2026.07 · Second refactoring (team of 2)', desc: 'Implemented the emotion timeline, custom video controls, search, and bookmarks; improved three-breakpoint responsive UI, accessibility, TanStack Query data flow, SSG with per-video OG metadata, and ErrorBoundary recovery. Defined AI-agent constraints in AGENTS.md, DESIGN.md, and an API skill, then reviewed results commit by commit.' }
        ],
        troubles: [
          { problem: 'Stored video duration differed from the real player duration, misaligning 100 emotion bins with playback', solution: 'Used the player duration as the source of truth and converted each bin center to seconds so the chart and seek position align.' },
          { problem: 'Seeking on every pointer move made timeline preview stutter', solution: 'Throttled seek calls to 200ms, always committed the final position, and preserved the previous play or pause state.' },
          { problem: 'Concurrent 401 responses caused duplicate token refreshes and inconsistent auth state', solution: 'Allowed only one refresh request, queued the rest, and retried them together with the renewed token.' }
        ],
        stack: ['React 19', 'TypeScript 5.9', 'Vite 7', 'TanStack Query v5', 'Zustand', 'Socket.IO', 'Nivo', 'SCSS', 'Vitest', 'Vercel', 'Flask 3', 'TensorFlow · OpenCV', 'Redis · MongoDB'],
        stackGroups: [
          { name: 'Frontend · my responsibility', items: ['React 19', 'TypeScript 5.9', 'Vite 7', 'TanStack Query v5', 'Zustand', 'Socket.IO Client', 'Nivo', 'SCSS', 'Vitest', 'Vercel'] },
          { name: 'Backend · AI pipeline', items: ['Flask 3', 'Flask-SocketIO', 'TensorFlow', 'OpenCV'] },
          { name: 'Data · infrastructure', items: ['MariaDB', 'MongoDB', 'Redis', 'Docker'] }
        ],
        outcome: 'Grand prize at Sejong University’s 16th Creative Design Contest in 2023. The refactored frontend was deployed to facereview.net in 2026.' },
      ...
      { id: 'p3', name: 'Haemadi', badge: '📔', desc: 'A journal for recording daily emotions and reflecting on their visualized flow', period: '2024.08 ~ 2024.11', team: 'Team of 3',
        list: { contribution: 'Journal flow and emotion-visualization UI · Zustand state structure', implementation: 'Centralized responsive branching · shared Framer Motion variants' },
        bullets: ['A diary service for recording and reflecting on daily emotions.', 'Each entry becomes a visual "node" in your sea of emotions, building a personal emotional map over time.', 'Maintainable responsive UI built with Zustand and CSS media queries.', 'Framer Motion animations make the act of journaling itself enjoyable.'],
        flow: ['Choose an emotion tag', 'Write the day', 'Visualize emotion in color and form', 'Review the monthly emotional flow'],
        features: [
          { name: 'Emotion journaling', desc: 'Record your day with emotion tags — step-by-step animations guide the writing flow' },
          { name: 'Emotion visualization', desc: 'Entries visualized by color and shape — a month of feelings at a glance' },
          { name: 'Responsive UI', desc: 'Layouts adapted for mobile, tablet, and desktop' },
          { name: 'Interaction animations', desc: 'Framer Motion page transitions and gesture-reactive motion' }
        ],
        roles: ['Built the frontend — journaling flow and emotion visualization UI', 'Designed Zustand state — centralized viewport logic in the store for maintainability', 'Built the animation system — shared Framer Motion variants for a consistent motion language'],
        troubles: [
          ...
          { problem: 'Page transition animations clashed with component unmounting, cutting motion off abruptly', solution: 'Guaranteed exit animations with AnimatePresence and defined shared variants for seamless transitions.' }
        ],
        stack: ['React', 'Zustand', 'Framer Motion'],
  ...
  ja: {
    ...
    blogTitle: '■ 技術ブログ',
    blogPosts: [],
    contactTitle: '■ チャンネル — メール · リポジトリ · ブログ',
    ...
    dShots: '実サービス画面',
    dOutcome: '確認済みの成果',
    dTodo: '※ 一部の詳細は例示として作成されています。リンク（mockアドレス）・スクリーンショット・詳細指標を実際の資料に差し替えてください。',
    projects: [
      { id: 'p1', name: 'フェイスレビュー', badge: '🏆', desc: 'Webカメラの表情を5感情に分類し、視聴反応と動画推薦につなげるサービス', period: '2023.10 ~ 2023.12 · 2026.01 ~ 2026.04 · 2026.07', team: '初期開発4人 · リファクタリング2人', verified: true,
        list: {
          contribution: '2023年フロントエンド・感情モデル学習 · 2026年フロントエンド刷新',
          implementation: '感情タイムライン・動画操作 · 検索/お気に入り · レスポンシブ/アクセシビリティ/SSG'
        },
        productSummary: 'YouTube視聴中にWebカメラのフレームをサーバーへ送り、AI Hubの感情認識データセットで直接学習したモデルが表情を「喜び・驚き・怒り・悲しみ・無表情」に分類します。区間・動画単位で蓄積した感情データは、現在区間の視聴者反応、感情タイムライン、個人統計、動画推薦に利用されます。',
        ...
        roles: [
          { title: '2023 · 初期開発（4人）', desc: 'Frontendとして参加し、サービスで使用する「喜び・驚き・怒り・悲しみ・無表情」の5感情分類モデルを直接学習しました。' },
          { title: '2026.01〜04 · 1次リファクタリング（2人）', desc: 'フロントエンドを担当し、改善方針と新機能を企画。CRA・Yarn ClassicをVite 7・Yarn Berry、React 19・TypeScript 5.9環境へ移行しました。' },
          { title: '2026.07 · 2次リファクタリング（2人）', desc: '感情タイムライン、カスタム動画操作、検索・お気に入りを実装し、3段階レスポンシブUI、アクセシビリティ、TanStack Query、SSG・動画別OG、ErrorBoundaryを整備しました。AGENTS.md・DESIGN.md・APIスキルでAIエージェントの作業基準を定め、コミット単位で成果物を検証しました。' }
        ],
        troubles: [
          { problem: '保存された動画時間と実際のプレイヤー時間が異なり、100感情区間と再生位置がずれる問題', solution: 'プレイヤーが返す実durationを基準に各区間の中心を秒単位へ変換し、グラフ時間軸とシーク位置を一致させました。' },
          { problem: 'タイムラインをドラッグするたびにシークが発生し、動画プレビューが不安定になる問題', solution: 'シーク呼び出しを200ms間隔に制限しつつ最後の位置を必ず反映し、操作前の再生・停止状態も維持しました。' },
          { problem: '複数APIが同時に401を受けるとトークン再発行が重複し、認証状態が競合する問題', solution: '再発行は1件だけ実行し、残りのリクエストを待機させた後、更新済みトークンでまとめて再試行しました。' }
        ],
        stack: ['React 19', 'TypeScript 5.9', 'Vite 7', 'TanStack Query v5', 'Zustand', 'Socket.IO', 'Nivo', 'SCSS', 'Vitest', 'Vercel', 'Flask 3', 'TensorFlow · OpenCV', 'Redis · MongoDB'],
        stackGroups: [
          { name: 'フロントエンド · 直接担当', items: ['React 19', 'TypeScript 5.9', 'Vite 7', 'TanStack Query v5', 'Zustand', 'Socket.IO Client', 'Nivo', 'SCSS', 'Vitest', 'Vercel'] },
          { name: 'バックエンド · AIパイプライン', items: ['Flask 3', 'Flask-SocketIO', 'TensorFlow', 'OpenCV'] },
          { name: 'データ · インフラ', items: ['MariaDB', 'MongoDB', 'Redis', 'Docker'] }
        ],
        outcome: '2023年に世宗大学第16回創意設計コンテストで大賞を受賞しました。2026年にはリファクタリングしたフロントエンドをfacereview.netへ公開しました。' },
      ...
      { id: 'p3', name: 'ヘマディ', badge: '📔', desc: '一日の感情を記録し、その流れを可視化して振り返る日記サービス', period: '2024.08 ~ 2024.11', team: 'チーム (3人)',
        list: { contribution: '日記作成・感情可視化UI · Zustand状態設計', implementation: 'レスポンシブ分岐の一元化 · 共通Framer Motion variants' },
        bullets: ['一日の感情を記録し、振り返ることができる感情管理日記サービスです。', '日記を書くと感情が海の「節目」として可視化され、時間とともに自分だけの感情マップが作られます。', 'ZustandとCSS media queryで保守可能なレスポンシブUIを実装しました。', 'Framer Motionでユーザー操作に応じた動的アニメーションを実装し、記録する体験自体が楽しくなるようにしました。'],
        flow: ['感情タグを選択', '一日を記録', '色と形で感情を可視化', '月間の感情推移を振り返る'],
        features: [
          { name: '感情日記の作成', desc: '感情タグと共に一日を記録 — 作成フローに合わせた段階別アニメーション' },
          { name: '感情の可視化', desc: '記録された感情を色と形で可視化し、一ヶ月の感情の流れを一目で確認' },
          { name: 'レスポンシブUI', desc: 'モバイル・タブレット・デスクトップに対応したレイアウト' },
          { name: 'インタラクションアニメーション', desc: 'Framer Motionによるページ遷移・ジェスチャー反応モーション' }
        ],
        roles: ['フロントエンド実装 — 日記作成フロー、感情可視化UI', 'Zustand状態設計 — ビューポート分岐ロジックをストアに一元化し保守性を確保', 'アニメーションシステム構築 — 共通variantsの定義で一貫したモーション言語を適用'],
        troubles: [
          { problem: '画面サイズの分岐が各コンポーネントに散在し、レスポンシブコードの保守が困難になった問題', solution: 'media queryの基準をトークン化し、Zustandストアでビューポート状態を一元管理して分岐の重複を排除しました。' },
          { problem: 'ページ遷移アニメーションがアンマウントと衝突しモーションが途切れる問題', solution: 'AnimatePresenceでexitアニメーションを保証し、共通variantsを定義して遷移が自然に繋がるようにしました。' }
        ],
        stack: ['React', 'Zustand', 'Framer Motion'],
...
const UI_COPY = {
  ko: {
    ...
    saveFile: 'SAVE FILE · 실제 성과',
    metrics: [
      { value: '대상', label: '창의설계경진대회' },
      { value: '약 20%', label: '서버 응답 개선' },
      { value: '5건', label: '실무 개선 퀘스트' }
    ],
    ...
    hud: {
      p1: ['클릭·시청 기록만으로는 영상 구간별 감정 반응을 알기 어려움', '2023 프론트엔드·5감정 모델 학습 · 2026 프론트엔드 리팩토링', '창의설계경진대회 대상 · facereview.net 배포'],
      p2: ['중계·채팅의 동시 업데이트', 'FE 아키텍처·SSR·실시간 UI', '실시간 문자 중계·경기별 채팅·SSR 페이지 구현'],
      p3: ['흩어진 반응형·모션 로직', '일기 플로우·상태·모션 시스템', '감정 일기·시각화·반응형 UI 구현']
    },
  ...
  en: {
    ...
    saveFile: 'SAVE FILE · PROVEN OUTCOMES',
    metrics: [
      { value: 'GRAND', label: 'Creative Design Prize' },
      { value: '~20%', label: 'Faster Server Response' },
      { value: '5', label: 'Workplace Improvements' }
    ],
    ...
    hud: {
      p1: ['Click and watch history alone cannot show emotional reactions by video segment', '2023 frontend and five-emotion model training · 2026 frontend refactoring', 'Creative Design grand prize · facereview.net deployed'],
      p2: ['Concurrent live feed and chat updates', 'FE architecture, SSR and real-time UI', 'Live play-by-play, per-game chat, and SSR pages implemented'],
      p3: ['Scattered responsive and motion logic', 'Journal flow, state and motion system', 'Emotion journal, visualization, and responsive UI implemented']
    },
  ...
  ja: {
    ...
    saveFile: 'SAVE FILE · 実績',
    metrics: [
      { value: '大賞', label: '創意設計コンテスト' },
      { value: '約20%', label: 'サーバー応答改善' },
      { value: '5件', label: '実務改善クエスト' }
    ],
    ...
    hudTitle: 'MISSION BRIEF', hudLabels: ['課題', '貢献', '成果'],
    imageLabels: {
    ...
    hud: {
      p1: ['クリック・視聴履歴だけでは動画区間ごとの感情反応が分からない', '2023年フロントエンド・5感情モデル学習 · 2026年フロントエンド刷新', '創意設計コンテスト大賞 · facereview.net公開'],
      p2: ['実況とチャットの同時更新', 'FE設計・SSR・リアルタイムUI', '文字中継・試合別チャット・SSRページを実装'],
      p3: ['分散したレスポンシブ・モーション処理', '日記フロー・状態・モーション設計', '感情日記・可視化・レスポンシブUIを実装']
    },
